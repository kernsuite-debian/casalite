...
#
        ...
        trec = casac.utils().torecord(pathname+'listcal.xml')

        casalog.origin('listcal')
        if trec.has_key('listcal') and casac.utils().verify(mytmp, trec['listcal']) :
            result = task_listcal.listcal(vis, caltable, field, antenna, spw, listfile, pagerows)
