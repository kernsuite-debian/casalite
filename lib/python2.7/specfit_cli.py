...
res = specif(imagename="myspectrum.im", ngauss=2, box="3,3,4,5", poly=2, multifit=true, wantreturn=True)
        ...
        self.__globals__['__last_task'] = 'specfit'
        self.__globals__['taskname'] = 'specfit'
        ###
        ...
        if useLocalDefaults :
           for item in function_signature_defaults.iteritems():
            ...
            myparams['ngauss'] = ngauss = self.parameters['ngauss']
            myparams['poly'] = poly = self.parameters['poly']
            ...
            myparams['amp'] = amp = self.parameters['amp']
            myparams['amperr'] = amperr = self.parameters['amperr']
            myparams['center'] = center = self.parameters['center']
            myparams['centererr'] = centererr = self.parameters['centererr']
            myparams['fwhm'] = fwhm = self.parameters['fwhm']
            myparams['fwhmerr'] = fwhmerr = self.parameters['fwhmerr']
            myparams['integral'] = integral = self.parameters['integral']
            myparams['integralerr'] = integralerr = self.parameters['integralerr']
            ...
            myparams['pampest'] = pampest = self.parameters['pampest']
            myparams['pcenterest'] = pcenterest = self.parameters['pcenterest']
            myparams['pfwhmest'] = pfwhmest = self.parameters['pfwhmest']
            myparams['pfix'] = pfix = self.parameters['pfix']
            myparams['gmncomps'] = gmncomps = self.parameters['gmncomps']
            myparams['gmampcon'] = gmampcon = self.parameters['gmampcon']
            myparams['gmcentercon'] = gmcentercon = self.parameters['gmcentercon']
            myparams['gmfwhmcon'] = gmfwhmcon = self.parameters['gmfwhmcon']
            myparams['gmampest'] = gmampest = self.parameters['gmampest']
            myparams['gmcenterest'] = gmcenterest = self.parameters['gmcenterest']
            myparams['gmfwhmest'] = gmfwhmest = self.parameters['gmfwhmest']
            myparams['gmfix'] = gmfix = self.parameters['gmfix']
            myparams['logfile'] = logfile = self.parameters['logfile']
            myparams['append'] = append = self.parameters['append']
            myparams['pfunc'] = pfunc = self.parameters['pfunc']
            myparams['goodamprange'] = goodamprange = self.parameters['goodamprange']
            myparams['goodcenterrange'] = goodcenterrange = self.parameters['goodcenterrange']
            myparams['goodfwhmrange'] = goodfwhmrange = self.parameters['goodfwhmrange']
...
#
        ...
        trec = casac.casac.utils().torecord(pathname+'specfit.xml')

        casalog.origin('specfit')
        try :
          #if not trec.has_key('specfit') or not casac.casac.utils().verify(mytmp, trec['specfit']) :
            #return False

          casac.casac.utils().verify(mytmp, trec['specfit'], True)
          scriptstr=['']
          ...
          if type(self.__call__.func_defaults) is NoneType:
              ...
              saveinputs('specfit', 'specfit.last', myparams, self.__globals__,scriptstr=scriptstr, do_save_inputs=do_full_logging)
          ...
          result = specfit(imagename, box, region, chans, stokes, axis, mask, ngauss, poly, estimates, minpts, multifit, model, residual, amp, amperr, center, centererr, fwhm, fwhmerr, integral, integralerr, wantreturn, stretch, logresults, pampest, pcenterest, pfwhmest, pfix, gmncomps, gmampcon, gmcentercon, gmfwhmcon, gmampest, gmcenterest, gmfwhmest, gmfix, logfile, append, pfunc, goodamprange, goodcenterrange, goodfwhmrange, sigma, outsigma)
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('specfit', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
#
    def defaults(self, param=None, ipython_globals=None, paramvalue=None, subparam=None):
        ...
        a['estimates'] = {
                    0:odict([{'value':''}, {'ngauss':1}, {'pampest':''}, {'pcenterest':''}, {'pfwhmest':''}, {'pfix':''}, {'pfunc':''}])}
        a['multifit'] = {
                    0:{'value':False}, 
                    1:odict([{'value':True}, {'amp':""}, {'amperr':""}, {'center':""}, {'centererr':""}, {'fwhm':""}, {'fwhmerr':""}, {'integral':""}, {'integralerr':""}])}
        a['gmncomps'] = {
                    0:odict([{'notvalue':0}, {'gmampcon':''}, {'gmcentercon':''}, {'gmfwhmcon':''}, {'gmampest':0.0}, {'gmcenterest':0.0}, {'gmfwhmest':0.0}, {'gmfix':''}])}
        a['logfile'] = {
...
#
#
    def description(self, key='specfit', subkey=None):
        desc={'specfit': 'Fit 1-dimensional gaussians and/or polynomial models to an image or image region',
               'imagename': 'Name of the input image',
               'box': 'Rectangular region to select in direction plane. Default is to use the entire direction plane.',
               'region': 'Region selection. Default is to use the full image.',
               'chans': 'Channels to use. Default is to use all channels.',
