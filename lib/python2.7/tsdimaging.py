...
import task_tsdimaging
def tsdimaging(infiles=[''], outfile='', overwrite=False, field='', spw='', antenna='', scan='', intent='OBSERVE_TARGET#ON_SOURCE', mode='channel', nchan=-1, start=0, width=1, veltype='radio', outframe='', gridfunction='BOX', convsupport=-1, truncate=-1, gwidth=-1, jwidth=-1, imsize=[
...
#
        ...
        trec = casac.utils().torecord(pathname+'tsdimaging.xml')

        casalog.origin('tsdimaging')
        if trec.has_key('tsdimaging') and casac.utils().verify(mytmp, trec['tsdimaging']) :
            result = task_tsdimaging.tsdimaging(infiles, outfile, overwrite, field, spw, antenna, scan, intent, mode, nchan, start, width, veltype, outframe, gridfunction, convsupport, truncate, gwidth, jwidth, imsize, cell, phasecenter, projection, pointingcolumn, restfreq, stokes, minweight, brightnessunit, clipminmax)
