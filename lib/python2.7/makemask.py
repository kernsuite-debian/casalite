...
#
        ...
        trec = casac.utils().torecord(pathname+'makemask.xml')

        casalog.origin('makemask')
        if trec.has_key('makemask') and casac.utils().verify(mytmp, trec['makemask']) :
            result = task_makemask.makemask(mode, inpimage, inpmask, output, overwrite, inpfreqs, outfreqs)
