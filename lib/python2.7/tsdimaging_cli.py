...
from types import *
from task_tsdimaging import tsdimaging
class tsdimaging_cli_:
    __name__ = "tsdimaging"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (tsdimaging_cli_,)
       self.__doc__ = self.__call__.__doc__

       self.parameters={'infiles':None, 'outfile':None, 'overwrite':None, 'field':None, 'spw':None, 'antenna':None, 'scan':None, 'intent':None, 'mode':None, 'nchan':None, 'start':None, 'width':None, 'veltype':None, 'outframe':None, 'gridfunction':None, 'convsupport':None, 'truncate':None, 'gwidth':None, 'jwidth':None, 'imsize':None, 'cell':None, 'phasecenter':None, 'projection':None, 'pointingcolumn':None, 'restfreq':None, 'stokes':None, 'minweight':None, 'brightnessunit':None, 'clipminmax':None, }
...
a weight image with the name outfile.weight.
        ...
        self.__globals__['__last_task'] = 'tsdimaging'
        self.__globals__['taskname'] = 'tsdimaging'
        ###
...
#
        ...
        trec = casac.casac.utils().torecord(pathname+'tsdimaging.xml')

        casalog.origin('tsdimaging')
        try :
          #if not trec.has_key('tsdimaging') or not casac.casac.utils().verify(mytmp, trec['tsdimaging']) :
            #return False

          casac.casac.utils().verify(mytmp, trec['tsdimaging'], True)
          scriptstr=['']
          ...
          if type(self.__call__.func_defaults) is NoneType:
              ...
              saveinputs('tsdimaging', 'tsdimaging.last', myparams, self.__globals__,scriptstr=scriptstr, do_save_inputs=do_full_logging)
          ...
          result = tsdimaging(infiles, outfile, overwrite, field, spw, antenna, scan, intent, mode, nchan, start, width, veltype, outframe, gridfunction, convsupport, truncate, gwidth, jwidth, imsize, cell, phasecenter, projection, pointingcolumn, restfreq, stokes, minweight, brightnessunit, clipminmax)
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('tsdimaging', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
...
#
#
    def description(self, key='tsdimaging', subkey=None):
        desc={'tsdimaging': 'SD task: imaging for total power and spectral data',
               'infiles': 'a list of names of input SD Measurementsets (only MS is allowed for this task)',
               'outfile': 'prefix of output images (.image, .weight, .sumwt, .psf)',
               'overwrite': 'overwrite the output file if already exists [True, False]',
               'field': 'select data by field IDs and names, e.g. \'3C2*\' (\'\'=all)',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        if a.has_key(paramname) :
              return a[paramname]
tsdimaging_cli = tsdimaging_cli_()
